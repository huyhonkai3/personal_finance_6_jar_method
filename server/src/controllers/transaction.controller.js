  ...
  try {
    await session.withTransaction(async () => {
      ...
      transaction = await createExpenseTransaction({
        ...
        matchedDictionaryRuleId:
          pendingExpense.matchedDictionaryRuleId ?? null,
        transactionDate: effectiveDate,
        source: "realtime",
        session,
      });
...
export async function updateTransactionJar(req, res) {
  ...
  if (!transaction || transaction.isDeleted) {
    throw new AppError(404, "TRANSACTION_NOT_FOUND", "Không tìm thấy giao dịch");
  }
  if (transaction.type !== "expense") {
    throw new AppError(422, "VALIDATION_ERROR", "Chỉ có thể đổi lọ cho giao dịch Chi tiêu");
  }
  ...
  const keyword = normalizeText(transaction.description || transaction.rawText || "");
  if (keyword) {
    await PersonalDictionaryRule.updateOne(
      { userId: req.userId, keyword },
      { $set: { jarId, sourceType: "user_correction" }, $inc: { matchCount: 1 } },
      { upsert: true },
    );
  }
...
export async function listTransactions(req, res) {
  const { periodId, jarId, type, dateFrom, dateTo, page = 1, limit = 20 } = req.query;
